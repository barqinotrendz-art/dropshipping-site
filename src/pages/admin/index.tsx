...
async function fetchDashboardStats(selectedCountry: string) {
  ...
  const filteredOrders = orders.filter(
    (order: any) => order.country === selectedCountry
  )
  console.log(filteredOrders)
  const filteredProducts = products.filter((product: any) => product.country === selectedCountry)
  ...
  const totalRevenue = filteredOrders
    .filter((order: any) => order.status?.toLowerCase() !== 'cancelled')
    .reduce(
      (sum: number, order: any) => sum + (order.totals?.grandTotal || 0),
      0
    )
...
const AdminDashboard: React.FC = () => {
  ...
  return (
    <div>
      ...
      {/* Stats Grid */}
      <div>
        <h3 className='md:text-[24px] font-bold text-gray-800 text-[18px] pb-3'>
          {selectedCountry}
        </h3>
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs sm:text-sm font-medium text-gray-600">
                Total Revenue {selectedCountry === "Saudi Arabia" ? "(ksa)" : "(uae)"}
              </p>
              <p className="text-lg sm:text-2xl font-bold text-green-600">
                {currency} {(stats?.totalRevenue || 0).toFixed(2)}
              </p>
            </div>
        ...
        <div className="bg-white p-3 sm:p-6 rounded-lg border shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs sm:text-sm font-medium text-gray-600">Total Orders {selectedCountry === "Saudi Arabia" ? "(ksa)" : "(uae)"}</p>
        ...
        <div className="bg-white p-3 sm:p-6 rounded-lg border shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs sm:text-sm font-medium text-gray-600">Total Products {selectedCountry === "Saudi Arabia" ? "(ksa)" : "(uae)"}</p>
      ...
      <div>
        <h3 className='md:text-[24px] font-bold text-gray-800 text-[18px] py-3'>Kingdom of SaudiArabia stats</h3>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8 mt-6 sm:mt-8">
        <div
          className={`rounded-lg p-4 sm:p-6 hover:shadow-md transition-shadow ${pendingOrdersCount > 0
        ...
        >
          <div className="flex items-start gap-3 sm:gap-4">
            <div
              className={`p-2 sm:p-3 rounded-full flex-shrink-0 ${pendingOrdersCount > 0 ? 'bg-red-100 animate-pulse' : 'bg-gray-100'
                }`}
            >
              <HiExclamationCircle
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                ...
                <span
                  className={`inline-flex items-center gap-2 px-2 sm:px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wide self-start ${pendingOrdersCount > 0 ? 'bg-red-600 text-white animate-pulse' : 'bg-gray-200 text-gray-600'
                    }`}
                >
              ...
              <p
                className={`text-xs sm:text-sm mt-2 leading-5 sm:leading-6 ${pendingOrdersCount > 0 ? 'text-red-600 font-medium' : 'text-gray-500'
                  }`}
              >
        ...
        <div
          className={`rounded-lg p-4 sm:p-6 hover:shadow-md transition-shadow ${lowStockCount > 0
            ? 'bg-red-50 border-2 border-red-300 animate-pulse'
            : 'bg-white border border-gray-200'
            }`}
        >
          <div className="flex items-start gap-3 sm:gap-4">
            <div
              className={`p-2 sm:p-3 rounded-full flex-shrink-0 ${lowStockCount > 0 ? 'bg-red-100 animate-pulse' : 'bg-gray-100'
                }`}
            >
              <HiTrendingDown
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                ...
                <span
                  className={`inline-flex items-center gap-2 px-2 sm:px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wide self-start ${lowStockCount > 0 ? 'bg-red-600 text-white animate-pulse' : 'bg-gray-200 text-gray-600'
                    }`}
                >
              ...
              <p
                className={`text-xs sm:text-sm mt-2 leading-5 sm:leading-6 ${lowStockCount > 0 ? 'text-red-600 font-medium' : 'text-gray-500'
                  }`}
              >
        ...
        <div
          className={`rounded-lg p-4 sm:p-6 hover:shadow-md transition-shadow ${pendingReviewsCount > 0
        ...
        >
          <div className="flex items-start gap-3 sm:gap-4">
            <div
              className={`p-2 sm:p-3 rounded-full flex-shrink-0 ${pendingReviewsCount > 0 ? 'bg-orange-100 animate-pulse' : 'bg-gray-100'
                }`}
            >
              <HiChatAlt
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                ...
                <span
                  className={`inline-flex items-center gap-2 px-2 sm:px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wide self-start ${pendingReviewsCount > 0 ? 'bg-orange-500 text-white animate-pulse' : 'bg-gray-200 text-gray-600'
                    }`}
                >
              ...
              <p
                className={`text-xs sm:text-sm mt-2 leading-5 sm:leading-6 ${pendingReviewsCount > 0 ? 'text-orange-600 font-medium' : 'text-gray-500'
                  }`}
              >
                ...
                <tbody>
                  {stats.recentOrders.map((order: any) => (
                    <tr key={order.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-3">
                        <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${order.status === 'completed' ? 'bg-green-100 text-green-800' :
