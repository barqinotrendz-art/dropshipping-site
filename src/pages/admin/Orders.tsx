...
const OrdersAdminPage: React.FC<OrdersAdminPageProps> = ({
  ...
  const orders = React.useMemo(() => {
    ...
    let filtered = allOrders.filter(
      (order) => order.country === selectedCountry
    )
  ...
  }, [allOrders, statusFilter, selectedCountry])
  ...
  const countryOrders = React.useMemo(() => {
  ...
  for (let i = 0; i < enhancedOrderItems.length; i++) {
    console.log(enhancedOrderItems[i], 'enh')
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
        ...
        <Link to={`${ADMIN_BASE_PATH}/orders/confirmed`} className="bg-blue-50 p-3 sm:p-4 rounded border hover:bg-blue-100 transition-colors cursor-pointer">
          <div className="text-lg sm:text-2xl font-bold text-blue-700">
            {countryOrders?.filter(o => o.status.toLowerCase() === 'confirmed').length || 0}
          </div>
        ...
        <Link to={`${ADMIN_BASE_PATH}/orders/shipped`} className="bg-indigo-50 p-3 sm:p-4 rounded border hover:bg-indigo-100 transition-colors cursor-pointer">
          <div className="text-lg sm:text-2xl font-bold text-indigo-700">
            {countryOrders?.filter(o => o.status.toLowerCase() === 'shipped').length || 0}
          </div>
        ...
        <Link to={`${ADMIN_BASE_PATH}/orders/delivered`} className="bg-green-50 p-3 sm:p-4 rounded border hover:bg-green-100 transition-colors cursor-pointer">
          <div className="text-lg sm:text-2xl font-bold text-green-700">
            {countryOrders?.filter(o => o.status.toLowerCase() === 'delivered').length || 0}
          </div>
        ...
        <Link to={`${ADMIN_BASE_PATH}/orders/cancelled`} className="bg-red-50 p-3 sm:p-4 rounded border hover:bg-red-100 transition-colors cursor-pointer">
          <div className="text-lg sm:text-2xl font-bold text-red-700">
            {countryOrders?.filter(o => o.status.toLowerCase() === 'cancelled').length || 0}
          </div>
        ...
        <Link to={`${ADMIN_BASE_PATH}/orders/refunded`} className="bg-gray-50 p-3 sm:p-4 rounded border hover:bg-gray-100 transition-colors cursor-pointer">
          <div className="text-lg sm:text-2xl font-bold text-gray-700">
            {countryOrders?.filter(o => o.status.toLowerCase() === 'refunded').length || 0}
          </div>
            ...
            <tbody>
              {orders?.map((order) => {
                ...
                return (
                  <tr
                    key={order.id}
                    className={`border-b hover:bg-gray-50 transition-colors ${isPending ? 'bg-yellow-50 border-l-4 border-l-yellow-500' : ''
                      }`}
                  >
        ...
        <div className="md:hidden">
          {orders?.map((order) => {
            ...
            return (
              <div
                key={order.id}
                className={`p-4 border-b last:border-b-0 ${isPending ? 'bg-yellow-50 border-l-4 border-l-yellow-500' : ''
                  }`}
              >
                              ...
                              <div className="flex  items-center justify-between bg-white rounded-lg p-3 border border-gray-200 shadow-sm">
                                <div className="flex items-center gap-4">
                                  ...
                                  <div className="text-center">
                                    ...
                                    <div className="font-semibold text-gray-700">{currency} {item.pricing?.[0]?.discountPrice.toFixed?.(2)}
                      ...
                      {enhancedOrderItems && enhancedOrderItems.length > 0 && (
                        <div className="bg-blue-50 rounded-lg p-3 border border-blue-200">
                          <div className="flex justify-between items-center text-sm">
                            ...
                            <span className="text-blue-800 font-medium">
                              Items Value: {currency || (selectedOrder?.country === "Sauid Arabia" ? "SAR" : "AED")} {selectedOrder.totals?.grandTotal}
                              {/* {enhancedOrderItems.reduce((sum, item) => sum + ((item.price || 0) * (item.qty || 0)), 0).toFixed(2)} */}
                            </span>
                          </div>
                        </div>
                      )}
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="space-y-3">
                    ...
                    {selectedOrder.totals?.shipping && (
                      <div className={`text-sm p-3 rounded border-l-4 ${['shipped', 'delivered'].includes((selectedOrder.status || '').toLowerCase())
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <div className="space-y-3">
                      {selectedOrder.timeline.map((entry, index) => {
                        ...
                        return (
                          <div key={index} className={`border-l-2 ${isLatestEntry ? 'border-blue-500' : 'border-gray-300'
